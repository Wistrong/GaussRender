...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(
        ...
        **kwargs
        ...
        ref_2d_hw = self.get_reference_points(
            tpv_h, tpv_w, dim="2d", bs=1, device="cpu"
        )
        ref_2d_zh = self.get_reference_points(
            tpv_z, tpv_h, dim="2d", bs=1, device="cpu"
        )
        ref_2d_wz = self.get_reference_points(
            tpv_w, tpv_z, dim="2d", bs=1, device="cpu"
        )
    ...
    def point_sampling(self, reference_points, pc_range, img_metas):
        ref2img = img_metas["projection_mat"]  # either lidar2img or ego2img
        ...
        ref2img = ref2img.view(1, B, num_cam, 1, 4, 4).repeat(D, 1, 1, num_query, 1, 1)

        reference_points_cam = torch.matmul(
            ref2img.to(torch.float32), reference_points.to(torch.float32)
        ).squeeze(-1)
        eps = 1e-5
        ...
        reference_points_cam[..., 0] /= img_metas["image_wh"][0][0][0]
        reference_points_cam[..., 1] /= img_metas["image_wh"][0][0][1]
